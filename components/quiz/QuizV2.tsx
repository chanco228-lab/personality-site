    ...
    if (nextIndex >= state.questions.length) {
      ...
      (async () => {
        const { data } = await supabase.from('results_v2').insert({
          type_id: typeId,
          ns_score: scores.NS, ha_score: scores.HA, rd_score: scores.RD,
          p_score: scores.P, sd_score: scores.SD, co_score: scores.CO,
          st_score: scores.ST, introvert_score: introvertScore,
        }).select().single();
  ...
  if (showNotice) {
    return (
      <div className="min-h-screen bg-bg flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-[480px] bg-yellow border-2 border-ink rounded-[24px] px-8 py-10 text-center" style={{ boxShadow: '10px 10px 0 #0E0E0E' }}>
          ...
          <button onClick={() => setShowNotice(false)} className="hero-cta w-full font-display font-black bg-ink text-paper border-2 border-ink rounded-full py-[18px] text-[18px]">
            診断を始める →
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-bg flex flex-col">
      <header className="sticky top-0 z-50 bg-paper border-b-2 border-ink">
        <div className="max-w-[720px] mx-auto px-4 py-3 flex items-center gap-3">
          <Link href="/" className="shrink-0">
            <span className="w-7 h-7 bg-yellow border-2 border-ink rounded-lg flex items-center justify-center font-mono text-[11px] font-bold" style={{ transform: 'rotate(-5deg)' }}>T7</span>
          </Link>
          <span className="font-mono text-[12px] font-bold shrink-0">Q{current} / {total}</span>
          <div className="flex-1 border-2 border-ink h-[8px] bg-paper overflow-hidden">
            <div className="h-full bg-yellow transition-all duration-500 ease-out" style={{ width: `${percentage}%` }} />
          </div>
          <span className="font-mono text-[12px] font-bold shrink-0 text-coral">{currentQuestion.factor}</span>
        </div>
      </header>
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-8">
        <div className={`w-full transition-all duration-300 ${isTransitioning ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
          <QuizCard key={state.currentIndex} questionText={currentQuestion.text} questionIndex={state.currentIndex} factor={currentQuestion.factor} onAnswer={handleAnswer} />
        </div>
      </main>
      <div className="px-4 pb-8">
        <div className="max-w-[640px] mx-auto flex items-center justify-between">
          <button onClick={handleBack} disabled={state.currentIndex === 0} className={`flex items-center gap-1 text-[13px] font-bold transition-all ${state.currentIndex === 0 ? 'text-ink/20 cursor-not-allowed' : 'text-ink hover:text-coral'}`}>← 戻る</button>
          <button onClick={handleRestart} className="text-[12px] font-mono text-ink/40 hover:text-ink transition-colors">やり直す</button>
        </div>
      </div>
    </div>
  );
