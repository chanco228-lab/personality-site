...
import { FACTOR_LABELS } from '@/data/types';
...
async function deleteV3LogStep(step: number) {
...
export default function QuizV3() {
  ...
  const handleAnswer = (value: number) => {
    ...
    deleteV3LogStep(state.currentIndex + 1).then(() => {
      logEvent('quiz_v3_step', state.currentIndex + 1, value);
    });
  ...
  const handleBack = () => {
    ...
    deleteV3LogStep(state.currentIndex);
  ...
  const factorLabel = FACTOR_LABELS[currentQuestion.factor] ?? currentQuestion.factor;
    ...
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-paper border-b-2 border-ink">
        <div className="max-w-[720px] mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <span className="font-mono text-[11px] font-bold shrink-0 text-coral hidden sm:block">
            {factorLabel}
          </span>
          <span className="font-mono text-[11px] font-bold shrink-0 text-coral sm:hidden">
